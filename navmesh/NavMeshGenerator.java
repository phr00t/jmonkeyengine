...
public class NavMeshGenerator implements Savable {
    ...
    private List<Geometry> findGeometries(Node node, ArrayList<Geometry> geoms) {
    ...
    public NavMesh completeCreationFromNode(Node n) {
        Mesh m = createFromNode(n);
        NavMesh nm = new NavMesh(m);
        return nm;
    }
    
    public Mesh createFromNode(Node n) {
        Mesh m = new Mesh();
        ArrayList<Geometry> list = new ArrayList<>();
    
        findGeometries(n, list);
        if(list.isEmpty() == false) {
            GeometryBatchFactory.mergeGeometries(list, m);
            return createFromMesh(m);                
        } else {
            return m;
        }
    }    
    
    public Mesh createFromMesh(Mesh mesh) {
        nmgen = new nmgen(cellSize, cellHeight, minTraversableHeight,
        ...
        TriangleMesh triMesh = nmgen.build(positions, indices, intermediateData); //buildNavMesh(positions, indices, intermediateData);
        if (triMesh == null) {
            return null;
        }
...
//            task.interrupt();
    ...
    public nmgen getNmgen() {
        return nmgen;
    }

    public void setNmgen(nmgen nmgen) {
        this.nmgen = nmgen;
    }
