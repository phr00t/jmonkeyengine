...
public class NavMesh implements Savable {
    ...
    /**
     * Only returns a cell if the point is inside the navmesh, if
    ...
    public float findInsideCellHeight(Vector3f point) {
        for(int i=0;i<cellList.size();i++) {
            Cell cell = cellList.get(i);
            if( cell.contains(point) ) {
                float height = cell.getHeightOnCell(point);
                if( point.y - height >= 0f ) return height;
            }
        }
        return -1f;
    }
    ...
    public Cell findClosestCell(Vector3f point) {
        ...
        for (Cell cell : cellList) {
            ...
            if (!foundHomeCell) {
                ...
                Cell.ClassifyResult Result = cell.classifyPathToCell(motionPath);
